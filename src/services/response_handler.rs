...
impl Response {
                    ...
                    match span.span {
                        Some(background) => {
                            for word in background {
                                let word_start_time = Self::convert_time_to_lrc_format(&word.begin.unwrap());
                                let word: String = format!("<{}> {} ", word_start_time, word.word.unwrap());
                                line.push_str(&word);
                            }
                        }
                        None => {
                            let word_start_time = Self::convert_time_to_lrc_format(&span.begin.unwrap());
                            let word: String = format!("<{}> {} ", word_start_time, span.word.unwrap());
                            line.push_str(&word);
                        },
                    }
                }
    ...
    fn add_spaces(synced_lyric_chars: Vec<char>, lyric_chars: &[char], lyric_char_index: &mut usize, word: &mut String) {
